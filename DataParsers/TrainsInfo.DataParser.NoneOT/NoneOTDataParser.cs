    ...
    public class NodeOTDataParser : IDataParser
    {
        ...
        public IList<RowValue> Parse(object data, IList<InfrastructureBase> infrastructures)
        {
            ...
            if (table != null)
            {
                ...
                var tableWithFilter = tableIAS_PYR_GP.Where(x => codesOperation.Contains(x.OperationCode) && x.StationCode == x.Index1 /*&& x.TrainNumber != 9999*/).ToList();
                foreach (var node in infrastructures.Where(x => x.Type == TypeInfrastructure.node).ToList())
                {
                    ...
                    result.Add(new RowValue(node.Station, OTTrain, (countTrain.Length < 2) ? countTrain.Insert(0, " ") : countTrain, DateTime.Now));
                }
            }
            //
            return result;
