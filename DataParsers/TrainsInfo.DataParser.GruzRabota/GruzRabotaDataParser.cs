...
{
    public class GruzRabotaDataParser : IDataParser
    {
        private readonly string rowPattern = @"\s*'.*'\s*;\s*'.*'\s*;\s*'.*'\s*;\s*'.*'\s*;\s*'.*'\s*;\s*'.*'\s*;\s*'.*'\s*$";
        ...
        public IList<RowValue> Parse(object data, IList<InfrastructureBase> infrastructures)
        {
            ...
            if (table != null)
            {
                ...
                //
                result.Add(new RowValue(Constants.KeyTableGruzRabota, parserValue.ToString(), table.LastUpdate));
            }
            //
            return result;
        }
